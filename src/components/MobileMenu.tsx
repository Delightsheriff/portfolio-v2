...
export default function MobileMenu() {
  ...
  return (
    <>
      ...
      <div
        className={`md:hidden fixed left-0 top-0 z-10 h-full w-full transform duration-[600ms] ease-[cubic-bezier(0.7,0,0,1)] dark:bg-gray-900 bg-white ${
        ...
        <nav className="flex flex-col mt-6">
          {data.map((link) => (
            <Link
              key={link.title}
              href={link.href}
              className="flex items-center gap-x-2 font-montserrat font-semibold text-lg dark:shadow-line-dark shadow-line-light p-6 group"
              onClick={onToggleNav}
            >
              <link.icon
                className="text-gray-500 group-hover:dark:text-white group-hover:text-gray-800 duration-300"
                aria-hidden="true"
              />
              {link.title}
            </Link>
